...
function ensureSupport(){
 const sidebar=document.querySelector('.sidebar');
 if(!sidebar||sidebar.querySelector('[data-support]'))return;
 const b=document.createElement('button');b.className='nav-item';b.type='button';b.textContent='Support';b.dataset.support='1';b.onclick=openSupport;
 const nav=sidebar.querySelector('.shell-nav');
...
async function openSupport(){
 ...
 if(!modal){modal=document.createElement('div');modal.id='supportModal';modal.className='support-modal';modal.innerHTML=`<div class="support-box"><h3>WhatsApp Support</h3><p>Select a support number to open WhatsApp chat.</p><div id="supportList" class="support-list"></div><button class="support-close" type="button" onclick="document.getElementById('supportModal')?.remove()">Close</button></div>`;document.body.appendChild(modal)}
 const list=$('supportList');list.innerHTML='<div class="account-msg">Loading...</div>';
 try{const x=await fetch('/api/support',{credentials:'same-origin',cache:'no-store'}),d=await x.json();if(!x.ok||!d.ok)throw new Error(d.error||'Unable to load support');const nums=d.whatsapp||[];list.innerHTML=nums.map((n,i)=>`<button type="button" onclick="window.open('https://wa.me/${String(n).replace(/[^0-9]/g,'')}','_blank','noopener')">WhatsApp Support ${i+1}<br><small>${esc(n)}</small></button>`).join('')||'<div class="account-msg">Support numbers are not configured yet.</div>'}catch(e){list.innerHTML='<div class="account-msg">Support is temporarily unavailable.</div>'}
}
function ensure(){
 style();ensureSupport();ensureMenuLogout();
 let section=$('account');
 ...
 section.innerHTML=`<div class="account-grid">
  <div class="account-card"><h3>Profile</h3><div class="account-sub">Your account information</div><div class="account-fields">
   ...
   <div class="account-field"><label>Unique Referral Code</label><input id="accountReferralCode" readonly></div>
  </div><div class="account-actions"><button class="account-btn" id="accountReferralCopy" type="button">Copy Referral Code</button><button class="account-btn" id="accountProfileSave" type="button">Save Profile</button></div><div id="accountProfileMsg" class="account-msg"></div></div>
 ...
 $('accountProfileSave').onclick=saveProfile;$('accountReferralCopy').onclick=copyReferralCode;$('accountSecuritySave').onclick=saveSecurity;$('accountPasswordSave').onclick=changePassword;$('accountHistoryFilter').onchange=loadHistory;
 return section
}
...
async function loadProfile(){const {r,d}=await json('/api/profile');if(!r.ok||!d.ok)return;$('accountFullName').value=d.profile.fullName||'';$('accountUsername').value=d.profile.username||'';$('accountEmail').value=d.profile.email||'';$('accountPhone').value=d.profile.phone||'';try{const x=await json('/api/referral/profile');if(x.r.ok&&x.d.ok){$('accountReferralCode').value=x.d.referral.code||'';$('accountReferralCopy').dataset.code=x.d.referral.code||''}}catch{}}
async function copyReferralCode(){const code=$('accountReferralCode')?.value||'';if(!code)return;try{await navigator.clipboard.writeText(code);$('accountProfileMsg').textContent='Referral code copied.'}catch{const el=$('accountReferralCode');el.select();document.execCommand('copy');$('accountProfileMsg').textContent='Referral code copied.'}}
