...
function ensure(){
 ...
 section.innerHTML=`<div class="account-grid">
  ...
  <div class="account-card"><h3>Logout</h3><div class="account-sub">Sign out of your TradeHub client session</div><button class="account-btn account-logout" id="accountLogout" type="button">Logout</button><div id="accountLogoutMsg" class="account-msg"></div></div>
 </div>`;
 section.dataset.ready='1';
 $('accountProfileSave').onclick=saveProfile;$('accountSecuritySave').onclick=saveSecurity;$('accountPasswordSave').onclick=changePassword;$('accountHistoryFilter').onchange=loadHistory;$('accountLogout').onclick=logout;
 return section
}
...
async function logout(){const msg=$('accountLogoutMsg');msg.textContent='Logging out...';const {r,d}=await json('/api/session/logout',{method:'POST',headers:{'Content-Type':'application/json'}});if(!r.ok||!d.ok){msg.textContent=d.error||'Logout failed.';return}location.href='/signin/'}
...
new MutationObserver(()=>{if($('account')&&!$('account').dataset.ready)ensure()}).observe(document.documentElement,{childList:true,subtree:true});
ensure();
