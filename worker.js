...
export default {
  async fetch(request, env) {
    ...
    if (['/withdrawal-ui.js','/account-ui.js','/activity-feed.js','/home-title-fix.js','/referral-ui.js','/home-rules.js','/home-cleanup.js','/rounds-final-ui.js'].includes(url.pathname)) {
...
async function withActivityFeed(response,env,pathname='/') {
  const html=await response.text(),scripts=[],isHome=pathname==='/'||pathname==='/index.html';
  ...
  if(!html.includes('/rounds-final-ui.js'))scripts.push('<script src="/rounds-final-ui.js?v=rounds-final-v2" defer></script>');
  ...
  const updated=html.replace('</body>',scripts.join('')+'</body>'),headers=new Headers(response.headers);
  headers.set('Cache-Control','no-store, no-cache, must-revalidate');headers.delete('content-length');
